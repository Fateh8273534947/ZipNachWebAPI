...
namespace QuickCheckPresentmentAPI.Controllers
{
    ...
    {
        [Route("api/ActivityStatus/FileActivityStatus")]
        [HttpPost]
        public loginResponse FileActivityStatus(SavePresentmentDetails ul)
        {
            ...
            else
            {
                ...
                if (Flag)
                {
                    ...
                    try
                    {
                        ...
                        dbcommand = new SqlCommand(query, conn);
                        dbcommand.CommandType = CommandType.StoredProcedure;
                        dbcommand.Parameters.AddWithValue("@QueryType", "EXISTINGACTIVITYSTATUS");
                        ...
                        if (ds.Tables[1].Rows.Count > 0 && ds.Tables[2].Rows.Count > 0)
                        {
                            ...
                            res.ResCode = "ykR30006";
                            res.ResDescription = "File Status Received Successfully";
                            ...
                            for (int i = 0; i < ds.Tables[2].Rows.Count; i++)
                            {
                                ...
                                GenerateXML = ValidatePresement.XMLCREATION(UMRN, AMOUNT, Convert.ToDateTime(ds.Tables[1].Rows[0]["LastEditDateTime"].ToString()).ToString("yyyy-MM-dd hh:mm"),Convert.ToString(ds.Tables[2].Rows[i]["UMRNStatus"]));
