...
{
    public class ActivityUpdateController : ApiController
    {
        [Route("api/ActivityUpdate/RecordUpdate")]
        [HttpPost]
        public loginResponse UMRNHold(SavePresentmentDetails ul)
        {
            ...
            else
            {
                ...
                if (Flag)
                {
                    ...
                    try
                    {
                        ...
                        dbcommand = new SqlCommand(query, conn);
                        dbcommand.CommandType = CommandType.StoredProcedure;
                        dbcommand.Parameters.AddWithValue("@QueryType", "EXISTINGACTIVITYUPDATE");
                        ...
                        DataSet ds = new DataSet();
                        da.Fill(ds);
                        int checkexist = Convert.ToInt32(ds.Tables[0].Rows[0]["exist"].ToString());
                        if (checkexist == 0)
                        {
                            res.ResDescription = "UMRN Not found";
                            res.Status = "Failure";
                            res.ActivityID = ul.ActivityID;
                            res.ResCode = "ykR30013";
                            res.FileNo = "";
                            res.BankCode = ul.BankCode;
                            res.CreatedDateTime = "";
                            res.LastEditDateTime = "";
                        ...
                        else
                        {
                            ...
                            {
                                res.ActivityID = ds.Tables[2].Rows[0]["ActivityId"].ToString();
                                res.FileNo = ds.Tables[2].Rows[0]["FileNo"].ToString();
                                res.BankCode = ds.Tables[2].Rows[0]["BankCode"].ToString();
                                res.CreatedDateTime = Convert.ToDateTime(ds.Tables[2].Rows[0]["CreatedDateTime"].ToString()).ToString("yyyy-MM-dd hh:mm");
                                res.LastEditDateTime = Convert.ToDateTime(ds.Tables[2].Rows[0]["LastEditDateTime"].ToString()).ToString("yyyy-MM-dd hh:mm");
                                res.TotalRecords = ds.Tables[1].Rows[0]["Recordcount"].ToString();
                                res.TotalAmt = ds.Tables[1].Rows[0]["TotalAmount"].ToString();
                                res.ResCode = "ykR30005";
                                res.Status = "Success";
                                res.FileStatus = ds.Tables[2].Rows[0]["FileStatus"].ToString();
                                res.ResDescription = "Record updated Successfully";
                                
                                string GenerateXML = ""; string UMRN = ""; string AMOUNT = "";
                                string finalxml = "<UMDATA>";
                                for (int i = 0; i < ds.Tables[3].Rows.Count; i++)
                                {
                                    UMRN = ds.Tables[3].Rows[i]["UMRN"].ToString();
                                    AMOUNT = ds.Tables[3].Rows[i]["AMOUNT"].ToString();
                                    GenerateXML = ValidatePresement.XMLCREATION(UMRN, AMOUNT, Convert.ToDateTime(ds.Tables[2].Rows[0]["LastEditDateTime"].ToString()).ToString("yyyy-MM-dd hh:mm"), ds.Tables[2].Rows[0]["FileStatus"].ToString());
